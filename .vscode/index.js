(function() {
    ...
    let text = `O ZordWay App destaca o tamanho, complexidade de sentenças e erros comuns; se você ver uma sentença em amarelo, a encurte ou a divida. Se você ver o destaque em vermelho, sua sentença é tão difícil e complicada que os seus leitores irão se perder tentando seguir sua lógica sinuosa e dividida - tente editar essa sentença para remover o destaque em vermelho.
    Você pode utilizar palavras mais curtas no lugar das palavras destacadas em roxo. Passe o cursor sobre ela para dicas.
    Advérbios e frases que enfraquecem seu texto são utilmente mostradas em azul. Remova elas ou escolha palavras mais fortes.
    Frases em verde estão sendo usadas para marcar a voz passiva.`;
    ...
    function counters() {
      ...
      document.querySelector(
        "#passive"
      ).innerHTML = `Você usou a voz passiva ${data.passiveVoice} vez${
        data.passiveVoice > 1 ? "es" : ""
      }. Tente usar ${Math.round(data.sentences / 5)} ou menos.`;
    ...
    function getPassive(sent) {
      ...
      let words = sent
        ...
        .toLowerCase()
        .split(" ");
      let ar = words.filter(word => word.match(/ar$/));
      if (ar.length > 0) {
        ar.forEach(match => {
    ...
    function checkPrewords(words, originalWords, match) {
      let preWords = ["é", "são", "era", "estamos", "ser", "estive", "sendo", "vão", "estão"];
